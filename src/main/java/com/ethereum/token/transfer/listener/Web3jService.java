...
//    private TOKEN token;
    ...
    public void init(long startBlockNumber, String url, String contractAddress, int retryTimes, int threadNumber, IBigBossAddrService bigBossAddrService) {
...
//        this.contractAddress = contractAddress.toLowerCase();
        ...
        log.info("init web3j success! " + "startBlockNumber: " +
                ...
                contractAddress + "retryTimes: " +
                retryTimes + "threadNumber: " +
                threadNumber);
...
//                addr, web3, manager, GAS_PRICE, BigInteger.valueOf(4_700_000));
    ...
    private boolean isSuccess(String txid, String contractAddr) {
        ...
        try {
            ...
            if (optional.isPresent()) {
                List<TOKEN.TransferEventResponse> responses = createToken(contractAddr).getTransferEvents(optional.get());
    ...
    private void handleTx(Transaction tx, long timestamp, ICallBack callBack, BigInteger blockNumber) {
        ...
        if (isContract(to)) {
            // 判断是否有这个币种
            if (to == null || bigBossAddrService.findByPrivateTokenAdd(to.toLowerCase()) == null) {
                return;
            }
            ...
            log.info("transfer token transaction:{From: " + from +
                    ...
                    ",Hash: " + tx.getHash() +
                    ",ContractAddress: " + to + "}");
            BigDecimal fromValue = Utils.valueTransfer(balanceOf(from, blockNumber, to.toLowerCase()), decimal);
            BigDecimal toValue = Utils.valueTransfer(balanceOf(inputData.getTo(), blockNumber, to.toLowerCase()), decimal);
    ...
    public BigInteger balanceOf(String addr, BigInteger blockNumber, String contractAddress) {
        int times = 0;
